...
**βMax** is a structured-data repair system designed for real-world scenarios where input records must satisfy strict format constraints but explicit format specifications (e.g., regular expressions or grammars) are unavailable. Existing repair techniques either rely on such specifications or require strong parser assumptions, limiting their applicability. In contrast, βMax performs format-free repair using only a collection of valid examples together with a black-box validator, inferring structural patterns directly from the data and optimizing repair candidates without requiring parser error locations or formal specifications. Across diverse real-world formats—including dates, times, ISBNs, IP addresses, and URLs—βMax repairs 83% of corrupted records, achieving a **1.77×** higher repair success rate than the state-of-the-art εRepair system while requiring **27.7×** fewer oracle calls. These results demonstrate that βMax provides an efficient and practical solution for data cleaning and recovery in environments where format specifications are unavailable.
